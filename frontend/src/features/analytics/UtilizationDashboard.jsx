    ...
    <div>
      ...
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
        {data.map((d, i) => {
          ...
          const x = 24 + i * ((width - 40) / data.length);
          return (
            <g key={i}>
              <rect x={x} y={chartH - h} width={barW} height={h} rx={barW / 4}
...
function AreaChart({ data = [], width = 300, height = 80, color = "#6366f1", label, showDots }) {
  ...
  const pts = data.map((d, i) => ({
    x: 10 + (i / (data.length - 1)) * (width - 20),
    y: height - 10 - ((d.value - minVal) / range) * (height - 24),
  }));
...
function MiniHeatmap({ heatmapData = [], compact }) {
  ...
  const sz = compact ? 4 : 6;
  const colors = ["#f1f5f9", "#c7d2fe", "#818cf8", "#6366f1", "#4338ca", "#312e81"];
          ...
          <div key={d} className="flex items-center gap-px mb-px">
            ...
            {row.map((val, h) => {
              ...
              return <div key={h} style={{ width: sz, height: sz, background: getColor(avg), borderRadius: 2 }}
                title={`${days[d]} ${h}:00 — ${avg.toFixed(1)}%`} className="cursor-help hover:ring-1 ring-indigo-400" />;
            })}
          </div>
        ))}
      </div>
...
function StatCard({ title, value, subtitle, icon, color = "#6366f1", trend }) {
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group relative overflow-hidden">
      ...
      <div className="absolute -right-4 -bottom-4 w-16 h-16 bg-indigo-500/5 rounded-full blur-xl group-hover:scale-150 transition-transform duration-700" />
    </div>
  );
}
  ...
  const renderCampusView = () => (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <StatCard title="Overall Avg" value={`${insights.overallAvg?.toFixed(1) || 0}%`} icon="📉" />
        <StatCard title="Peak Load" value={insights.peakHour?.name || "-"} subtitle={`${insights.peakHour?.value?.toFixed(1)}% density`} icon="⚡" />
        <StatCard title="Busiest Day" value={insights.busiestDay?.name || "-"} subtitle={`${insights.busiestDay?.value?.toFixed(1)}% load`} icon="🏢" />
        <StatCard title="Total Capacity" value={facList?.reduce((s,f) => s + (f.capacity || 0), 0)} icon="👥" subtitle="Active Slots" />
        <StatCard title="Status Health" value={`${((insights.statusSegments?.find(s=>s.name==="Active")?.value || 0) / (facList?.length || 1) * 100).toFixed(0)}%`} icon="🛡️" subtitle="Online Fleet" />
        <StatCard title="Report Period" value={`${Math.round((new Date(dateRange.to) - new Date(dateRange.from)) / (1000 * 60 * 60 * 24))} Days`} icon="📅" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-4 flex flex-col gap-4">
          <div className="bg-indigo-600 rounded-3xl p-6 text-white shadow-xl shadow-indigo-100 flex-1 flex flex-col justify-between">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-indigo-200 mb-2">Automated Optimization</p>
              <p className="text-sm font-bold leading-snug">
                {analytics?.underutilizedFacilities?.length > 3 
                  ? "Load imbalance detected. Recommend shift of core lectures to Zone B." 
                  : "Campus optimization optimal. System stable."}
              </p>
            </div>
            <div className="mt-4 pt-4 border-t border-white/10">
              <div className="flex justify-between text-[10px] font-black text-indigo-200">
                <span>CONCURRENCY</span>
                <span>{(insights.overallAvg * 1.2).toFixed(1)}% MAX</span>
              </div>
              <div className="w-full h-1 bg-white/10 rounded-full mt-1 overflow-hidden">
                <div className="h-full bg-white rounded-full" style={{ width: `${insights.overallAvg}%` }} />
              </div>
            </div>
          </div>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          <StatCard title="30-Day Avg" value={`${u30}%`} icon="📆" />
          <StatCard title="7-Day Avg" value={`${u7}%`} trend={trendPct} icon="📉" />
          <StatCard title="Peak Hour" value={fPeakHour.name} subtitle={`${fPeakHour.value.toFixed(1)}%`} icon="🔥" />
          <StatCard title="Quietest" value={fQuietHour.name} subtitle={`${fQuietHour.value.toFixed(1)}%`} icon="🌙" />
          <StatCard title="Spots Free" value={ins.spots_available ?? "—"} icon="💺" />
          <StatCard title="Next Booking" value={ins.next_booking_time ? ins.next_booking_time.slice(0, 5) : "None"} icon="⏰" />
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.15em] text-indigo-300">Live</span>
            </div>
