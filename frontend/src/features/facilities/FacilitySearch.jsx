      ...
      <div className="bg-transparent">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[320px_1fr] gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24 self-start">
            <h2 className="text-2xl font-bold tracking-tight mb-4 text-[#0f172a]">Filters</h2>
            ...
            <form onSubmit={search} className="space-y-4">
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#334155] mb-1">Minimum Capacity</label>
                  <input type="number" min="1" value={minCapacity} onChange={(e) => setMinCapacity(e.target.value)} placeholder="e.g., 50" className="w-full px-3 py-2 border border-[#e2e8f0] rounded-xl shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-[#334155] mb-1">Location</label>
                  <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g., Ground Floor" className="w-full px-3 py-2 border border-[#e2e8f0] rounded-xl shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                </div>

                <div>
                  <label className="block text-sm font-medium text-[#334155] mb-1">Building</label>
                  <input type="text" value={building} onChange={(e) => setBuilding(e.target.value)} placeholder="e.g., Building A" className="w-full px-3 py-2 border border-[#e2e8f0] rounded-xl shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                </div>
                ...
                {isSuggestionMode && (
                  <div className="space-y-4 p-4 bg-indigo-50/50 rounded-xl border border-indigo-100 animate-in slide-in-from-top-2 duration-300">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#64748b] uppercase mb-1">From</label>
                      <input 
                        type="datetime-local" 
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-[#64748b] uppercase mb-1">Until</label>
                      <input 
                        type="datetime-local" 
                        value={suggestionEnd} 
              ...
              <div className="flex gap-2">
                <button type="submit" disabled={loading} className="flex-1 px-4 py-2.5 bg-[#0f172a] hover:bg-[#1e293b] text-white font-semibold rounded-xl transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">{loading ? "Searching..." : "🔍 Search"}</button>
                <button type="button" onClick={handleClear} className="px-6 py-2.5 bg-[#f1f5f9] hover:bg-slate-200 text-[#334155] font-semibold rounded-xl transition-all duration-200 active:scale-95">Clear</button>
              </div>
            </form>
          </div>
          ...
          <div className="flex flex-col">
              <div className="bg-white rounded-2xl shadow-sm">
                <div className="p-6">
                <h3 className="text-2xl font-bold tracking-tight mb-2 text-[#0f172a]">Facilities ({results.length})</h3>
                {results.length > 0 && (
                  <p className="text-sm text-[#475569] mb-4">Click <span className="font-medium">Select</span> to create a booking.</p>
                )}

                {results.length === 0 ? (
                  <p className="text-[#475569]">No facilities found matching your criteria.</p>
                ) : (
                  <div className="space-y-3">
                    {results.map((facility, idx) => (
                      <div key={facility.id ?? facility.facilityId ?? facility.name ?? idx} className="border border-[#e2e8f0] rounded-2xl p-4 hover:shadow-sm transition-shadow">
                        <div className="flex justify-between items-start gap-4">
                        <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h4 className="text-lg font-semibold text-[#0f172a]">{facility.name}</h4>
                            ...
                            <div className="grid grid-cols-2 gap-x-6 gap-y-1 mt-2 text-[13px] text-[#475569]">
                              ...
                              <div><span className="font-medium">Capacity:</span> <span className="text-[#0f172a] font-black">{facility.capacity}</span></div>
                            ...
                            {facility.isSmartMatch && (
                              <div className="mt-3 bg-[#f8fafc] rounded-2xl p-2 border border-[#f1f5f9] flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-[10px] text-[#94a3b8] font-bold uppercase">{Math.round(facility.utilizationScore)}% Opt.</span>
                                </div>
                          ...
                          <button type="button" onClick={() => handleSelectFacility(facility)} className="px-5 py-2.5 bg-[#0f172a] hover:bg-[#1e293b] text-white font-semibold rounded-xl transition-all duration-200 active:scale-95 whitespace-nowrap self-center">Select</button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="p-4 flex items-center justify-between">
                <div className="text-sm text-[#475569]">Showing {startIndex}-{endIndex} of {total}</div>

                <nav className="inline-flex items-center space-x-2" role="navigation" aria-label="Pagination">
                  <button onClick={() => page > 0 && loadPage(page - 1, size)} disabled={page <= 0} className="inline-flex items-center justify-center h-8 w-8 rounded-xl bg-white border border-[#e2e8f0] hover:bg-[#f8fafc] disabled:opacity-50" aria-label="Previous page">
                  ...
                  {(() => {
                    ...
                    if (pages[0] > 0) {
                      nodes.push(
                        <button key="p0" onClick={() => loadPage(0, size)} className="h-8 min-w-[36px] px-2 rounded-xl bg-white border border-[#e2e8f0] text-sm hover:bg-[#f8fafc]">1</button>,
                      );
                      if (pages[0] > 1) nodes.push(<span key="e1" className="px-2 text-[#94a3b8]">…</span>);
                    }

                    pages.forEach((p) => {
                      nodes.push(
                        <button key={p} onClick={() => loadPage(p, size)} className={`h-8 min-w-[36px] px-2 rounded-xl text-sm ${p === page ? 'bg-[#0f172a] text-white shadow' : 'bg-white border border-[#e2e8f0] hover:bg-[#f8fafc]'}`}>
                          {p + 1}
                        </button>,
                      );
                    });
                    ...
                    if (pages[pages.length - 1] < totalPages - 1) {
                      if (pages[pages.length - 1] < totalPages - 2) nodes.push(<span key="e2" className="px-2 text-[#94a3b8]">…</span>);
                      nodes.push(<button key="plast" onClick={() => loadPage(totalPages - 1, size)} className="h-8 min-w-[36px] px-2 rounded-xl bg-white border border-[#e2e8f0] text-sm hover:bg-[#f8fafc]">{totalPages}</button>);
                    }

                    return nodes;
                  })()}

                  <button onClick={() => (page + 1) < totalPages && loadPage(page + 1, size)} disabled={(page + 1) >= totalPages} className="inline-flex items-center justify-center h-8 w-8 rounded-xl bg-white border border-[#e2e8f0] hover:bg-[#f8fafc] disabled:opacity-50" aria-label="Next page">
                    <span className="text-[#334155] text-lg">›</span>
                  </button>
                </nav>
              </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6">
        <h2 className="text-2xl font-bold tracking-tight mb-4 text-[#0f172a]">Search Facilities</h2>
        ...
        <form onSubmit={search} className="space-y-4">
                ...
                {isSuggestionMode && (
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-indigo-50 border border-indigo-100 rounded-xl animate-in slide-in-from-top-2">
                    <div>
                      ...
                      <input type="datetime-local" value={suggestionStart} onChange={(e) => setSuggestionStart(e.target.value)} className="w-full px-3 py-2 border border-indigo-200 rounded-xl shadow-sm text-sm" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-black text-indigo-600 uppercase mb-1">Until</label>
                      <input type="datetime-local" value={suggestionEnd} onChange={(e) => setSuggestionEnd(e.target.value)} className="w-full px-3 py-2 border border-indigo-200 rounded-xl shadow-sm text-sm" />
                    </div>
                    <div className="flex items-end">
                      <button type="button" onClick={handleSuggest} disabled={loading} className="w-full py-2 bg-indigo-600 text-white rounded-xl font-bold shadow-lg hover:bg-indigo-700 transition">Get Matches</button>
                    </div>
                  </div>
                )}
            </div>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={loading} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-[#0f172a] to-[#0f172a] hover:from-[#1e293b] hover:to-[#1e293b] text-white font-semibold rounded-xl shadow-sm hover:shadow-lg transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:from-[#0f172a] disabled:hover:to-[#0f172a]">{loading ? "Searching..." : "🔍 Search"}</button>
            <button type="button" onClick={handleClear} className="px-6 py-2.5 bg-[#f1f5f9] hover:bg-slate-200 text-[#334155] font-semibold rounded-xl border border-[#e2e8f0] shadow-sm hover:shadow-sm transition-all duration-200 active:scale-95">Clear</button>
          </div>
        </form>
      </div>
      ...
      {(hasSearched || results.length > 0) && (
        <div className="bg-white rounded-2xl shadow-sm">
          <div className="p-6">
            <h3 className="text-xl font-bold mb-2 text-[#0f172a]">Results ({results.length})</h3>

            {results.length > 0 && (
              <p className="text-sm text-[#475569] mb-4">Click the <span className="font-medium">"Select"</span> button for a facility to proceed with booking on the right.</p>
            )}
          </div>
          ...
          <div className="p-4 flex items-center justify-between">
            ...
            <nav className="inline-flex items-center space-x-2" role="navigation" aria-label="Pagination">
              ...
              {(() => {
                ...
                if (pages[0] > 0) {
                  nodes.push(
                    <button key="p0b" onClick={() => loadPage(0, size)} className="h-8 min-w-[36px] px-2 rounded-xl bg-white border border-[#e2e8f0] text-sm hover:bg-[#f8fafc]">1</button>,
                  );
                  if (pages[0] > 1) nodes.push(<span key="e1b" className="px-2 text-[#94a3b8]">…</span>);
                }

                pages.forEach((p) => {
                  nodes.push(
                    <button key={p + 'b'} onClick={() => loadPage(p, size)} className={`h-8 min-w-[36px] px-2 rounded-xl text-sm ${p === page ? 'bg-[#0f172a] text-white shadow' : 'bg-white border border-[#e2e8f0] hover:bg-[#f8fafc]'}`}>
                      {p + 1}
                    </button>,
                  );
                });
                ...
                if (pages[pages.length - 1] < totalPages - 1) {
                  if (pages[pages.length - 1] < totalPages - 2) nodes.push(<span key="e2b" className="px-2 text-[#94a3b8]">…</span>);
                  nodes.push(<button key="plastb" onClick={() => loadPage(totalPages - 1, size)} className="h-8 min-w-[36px] px-2 rounded-xl bg-white border border-[#e2e8f0] text-sm hover:bg-[#f8fafc]">{totalPages}</button>);
                }
